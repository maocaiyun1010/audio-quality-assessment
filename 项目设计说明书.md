...
| 项目 | 内容 |
| --- | --- |
| 文档版本 | **1.6**（侧栏历史预览：``dual_webui_*`` 等分桶筛选；``eval_source_summary`` 补充 ``dual_webui`` 与 ``analysis`` 配对说明） |
| 代码根目录 | **项目根目录**（本仓库交付包名如 `audio_evaluation_projectV1.1`；`config` / `speaker_eval.settings.paths.BASE_DIR` 默认解析为该目录，可用环境变量 **`SPEAKER_BASE_DIR`** 整体迁移） |
| 运行形态 | 本地 Python 流水线；**Streamlit Web UI**；可选 **FastAPI**；**无** Docker / **无** Dify Agent 内嵌 |
| 用户向说明 | 同目录 **[使用说明.md](./使用说明.md)**（安装、启动、Web/CLI/HTTP、环境变量与排障） |
| 修订记录 | 见文末 **附录 A** |
...
│   └── recording_config.py           # 录音采样率等工程级常量（经 settings 导出）│   └── run_all.py                    # CLI 转发；含 Web UI 调用的 main_run_eval、时长/增益补丁等
│
├── ⚙️  配置中心
│
├── 🎛️  设备与角色管理
...
1. **环境**：Python 3.10+；依赖含 `adbutils`、`sounddevice`、`soundfile`、`requests`、`python-docx`、`streamlit`、`pandas`、`matplotlib`、`fastapi`（若启用服务）等（见 `requirements.txt`）。
2. **ADB**：`adb devices` 中目标为 `device`。
3. **音源**：放入 `assets/test_audio`。
4. **Dify**：配置密钥与工作流 URL；模型支持音频附件。
5. **执行（任选）**
   - 双击 **`启动程序.bat`**：菜单 **1** CLI / **2** HTTP / **3** Web UI；
   - 双击 **`启动WebUI.bat`**；
   - 命令行：`python run_all.py`、`python local_service.py`、`python -m streamlit run web_ui.py`。
6. **CI / 计划任务**：CLI 加 `--yes`，并预设 `SPEAKER_RECORD_TOOL`、`DIFY_*` 等。
