...
| 程序版本 | **V1.3**（外发压缩包目录名：`喇叭音质测评系统_V1.3_YYYYMMDD`） |
...
| **外发交付** | 解压 **`喇叭音质测评系统_V1.3_*.zip`** → 双击 **`一键安装依赖.bat`** 或 **`启动WebUI.bat`**（详见 **[外发使用说明.md](./外发使用说明.md)**） |
...
1. 解压 **`喇叭音质测评系统_V1.3_YYYYMMDD.zip`** 至本机目录（勿复制他人 `.venv`）。
...
产物写入 **`dist/`**：同名文件夹 + **`喇叭音质测评系统_V1.3_YYYYMMDD.zip`**。可选参数：`--out-dir`、`--no-zip`（仅文件夹）。打包前勿将含密钥的 JSON 放入将被复制的路径；脚本已排除常见本地密钥与用户状态文件。
